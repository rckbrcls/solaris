...
struct PhotoEditorFilters: View {
    ...
    private func renderThumbnail(from image: UIImage, with state: PhotoEditState, maxSize: CGFloat) -> UIImage? {
        ...
        if state.vignette > 0.0, let radial = CIFilter(name: "CIRadialGradient") {
            let v = CGFloat(min(max(state.vignette, 0.0), 1.0))
            ...
            let innerRatio = 0.85 - 0.30 * Double(v) // shrink inner as intensity grows
            ...
            if var overlay = radial.outputImage?.cropped(to: output.extent) {
                ...
                if let over = CIFilter(name: "CISourceOverCompositing") {
                    over.setValue(overlay, forKey: kCIInputImageKey)
                    over.setValue(output, forKey: kCIInputBackgroundImageKey)
                    if let o = over.outputImage { output = o }
                }
            }
        }
        // Invert
        ...
        if state.grain > 0.0 {
            ...
            let shaped = CGFloat(pow(Double(baseK), 0.85))
            if let noise = CIFilter(name: "CIRandomGenerator")?.outputImage?.cropped(to: output.extent) {
                ...
                // Overlay blend then lerp by shaped
                ...
                if shaped >= 1.0 {
                    ...
                    let mask = CIImage(color: CIColor(red: 0, green: 0, blue: 0, alpha: shaped)).cropped(to: output.extent)
    ...
    private func renderThumbnailMetalPetal(from image: UIImage, with state: PhotoEditState, maxSize: CGFloat) -> UIImage? {
        ...
        // Vignette (corrigido para qualquer aspecto)
        if state.vignette > 0.0 {
            ...
            if let radial = CIFilter(name: "CIRadialGradient") {
                ...
                if var overlayCI = radial.outputImage?.cropped(to: extent) {
                    // Smoothstep na alpha
                    if let poly = CIFilter(name: "CIColorPolynomial") {
        ...
        // Film Grain using MTI + CI noise (symmetric B/W via overlay)
        if state.grain > 0.0 {
            let baseK = max(0.0, min(1.0, state.grain * 10.0))
            ...
            if let rand = CIFilter(name: "CIRandomGenerator")?.outputImage?.cropped(to: extent) {
                ...
                // Overlay blend directly in sRGB for speed (OK for thumbnails)
